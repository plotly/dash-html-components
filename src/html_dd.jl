...
function html_dd(; kwargs...)
        ...
        result = Component("Dd", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_dd"))
            end
...
function html_dd(children::Any; kwargs...)
    result = html_dd(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_dd(children_maker::Function; kwargs...) = html_dd(children_maker(); kwargs...)
