...
function html_h6(; kwargs...)
        ...
        result = Component("H6", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_h6"))
            end
...
function html_h6(children::Any; kwargs...)
    result = html_h6(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_h6(children_maker::Function; kwargs...) = html_h6(children_maker(); kwargs...)
