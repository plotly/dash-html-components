...
"""
    html_nextid(;kwargs...)
    html_nextid(children::Any;kwargs...)
    html_nextid(children_maker::Function;kwargs...)
...
function html_nextid(; kwargs...)
        ...
        result = Component("Nextid", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_nextid"))
            end
...
function html_nextid(children::Any; kwargs...)
    result = html_nextid(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_nextid(children_maker::Function; kwargs...) = html_nextid(children_maker(); kwargs...)
