...
function html_track(; kwargs...)
        available_props = Symbol[:children, :id, :n_clicks, :n_clicks_timestamp, :key, :role, :default, :kind, :label, :src, :srcLang, :accessKey, :className, :contentEditable, :contextMenu, :dir, :draggable, :hidden, :lang, :spellCheck, :style, :tabIndex, :title, :loading_state]
        wild_props = Symbol[Symbol("data-"), Symbol("aria-")]
        return Component("html_track", "Track", "dash_html_components", available_props, wild_props; kwargs...)
end

html_track(children::Any; kwargs...) = html_track(;kwargs..., children = children)
