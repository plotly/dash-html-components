...
"""
    html_element(;kwargs...)
    html_element(children::Any;kwargs...)
    html_element(children_maker::Function;kwargs...)
...
function html_element(; kwargs...)
        ...
        result = Component("Element", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_element"))
            end
...
function html_element(children::Any; kwargs...)
    result = html_element(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_element(children_maker::Function; kwargs...) = html_element(children_maker(); kwargs...)
