...
function html_objectel(; kwargs...)
        available_props = Symbol[:children, :id, :accessKey, :className, :contentEditable, :contextMenu, :data, :dir, :draggable, :form, :height, :hidden, :key, :lang, :loading_state, :n_clicks, :n_clicks_timestamp, :name, :role, :spellCheck, :style, :tabIndex, :title, :type, :useMap, :width]
        wild_props = Symbol[Symbol("aria-"), Symbol("data-")]
        return Component("html_objectel", "ObjectEl", "dash_html_components", available_props, wild_props; kwargs...)
end

html_objectel(children::Any; kwargs...) = html_objectel(;kwargs..., children = children)
