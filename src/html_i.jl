...
function html_i(; kwargs...)
        ...
        result = Component("I", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_i"))
            end
...
function html_i(children::Any; kwargs...)
    result = html_i(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_i(children_maker::Function; kwargs...) = html_i(children_maker(); kwargs...)
