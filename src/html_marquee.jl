...
function html_marquee(; kwargs...)
        available_props = Symbol[:children, :id, :n_clicks, :n_clicks_timestamp, :key, :role, :loop, :accessKey, :className, :contentEditable, :contextMenu, :dir, :draggable, :hidden, :lang, :spellCheck, :style, :tabIndex, :title, :loading_state]
        wild_props = Symbol[Symbol("data-"), Symbol("aria-")]
        return Component("html_marquee", "Marquee", "dash_html_components", available_props, wild_props; kwargs...)
end

html_marquee(children::Any; kwargs...) = html_marquee(;kwargs..., children = children)
