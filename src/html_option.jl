...
- `selected` (a value equal to: 'selected', 'SELECTED' | Bool; optional): Defines a value which will be selected on page load.
...
"""
function html_option(; kwargs...)
        available_props = Set(Symbol[:children, :id, :n_clicks, :n_clicks_timestamp, :key, :role, :disabled, :label, :selected, :value, :accessKey, :className, :contentEditable, :contextMenu, :dir, :draggable, :hidden, :lang, :spellCheck, :style, :tabIndex, :title, :loading_state])
        wild_props = Set(Symbol[Symbol("data-"), Symbol("aria-")])
        wild_regs = r"^(?<prop>data-|aria-)"

        result = Component("Option", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_option"))
            end
...
function html_option(children::Any; kwargs...)
    result = html_option(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_option(children_maker::Function; kwargs...) = html_option(children_maker(); kwargs...)
