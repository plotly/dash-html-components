...
function html_base(; kwargs...)
        available_props = Symbol[:children, :id, :n_clicks, :n_clicks_timestamp, :key, :role, :href, :target, :accessKey, :className, :contentEditable, :contextMenu, :dir, :draggable, :hidden, :lang, :spellCheck, :style, :tabIndex, :title, :loading_state]
        wild_props = Symbol[Symbol("data-"), Symbol("aria-")]
        return Component("html_base", "Base", "dash_html_components", available_props, wild_props; kwargs...)
end

html_base(children::Any; kwargs...) = html_base(;kwargs..., children = children)
