...
function html_time(; kwargs...)
        available_props = Symbol[:children, :id, :n_clicks, :n_clicks_timestamp, :key, :role, :dateTime, :accessKey, :className, :contentEditable, :contextMenu, :dir, :draggable, :hidden, :lang, :spellCheck, :style, :tabIndex, :title, :loading_state]
        wild_props = Symbol[Symbol("data-"), Symbol("aria-")]
        return Component("html_time", "Time", "dash_html_components", available_props, wild_props; kwargs...)
end

html_time(children::Any; kwargs...) = html_time(;kwargs..., children = children)
