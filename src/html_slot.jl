...
function html_slot(; kwargs...)
        ...
        result = Component("Slot", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_slot"))
            end
...
function html_slot(children::Any; kwargs...)
    result = html_slot(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_slot(children_maker::Function; kwargs...) = html_slot(children_maker(); kwargs...)
