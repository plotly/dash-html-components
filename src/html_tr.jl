...
function html_tr(; kwargs...)
        ...
        result = Component("Tr", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_tr"))
            end
...
function html_tr(children::Any; kwargs...)
    result = html_tr(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_tr(children_maker::Function; kwargs...) = html_tr(children_maker(); kwargs...)
