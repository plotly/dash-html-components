...
function html_embed(; kwargs...)
        available_props = Symbol[:children, :id, :n_clicks, :n_clicks_timestamp, :key, :role, :height, :src, :type, :width, :accessKey, :className, :contentEditable, :contextMenu, :dir, :draggable, :hidden, :lang, :spellCheck, :style, :tabIndex, :title, :loading_state]
        wild_props = Symbol[Symbol("data-"), Symbol("aria-")]
        return Component("html_embed", "Embed", "dash_html_components", available_props, wild_props; kwargs...)
end

html_embed(children::Any; kwargs...) = html_embed(;kwargs..., children = children)
