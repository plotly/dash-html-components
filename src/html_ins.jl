...
function html_ins(; kwargs...)
        available_props = Symbol[:children, :id, :n_clicks, :n_clicks_timestamp, :key, :role, :cite, :dateTime, :accessKey, :className, :contentEditable, :contextMenu, :dir, :draggable, :hidden, :lang, :spellCheck, :style, :tabIndex, :title, :loading_state]
        wild_props = Symbol[Symbol("data-"), Symbol("aria-")]
        return Component("html_ins", "Ins", "dash_html_components", available_props, wild_props; kwargs...)
end

html_ins(children::Any; kwargs...) = html_ins(;kwargs..., children = children)
