...
function html_data(; kwargs...)
        available_props = Symbol[:children, :id, :n_clicks, :n_clicks_timestamp, :key, :role, :value, :accessKey, :className, :contentEditable, :contextMenu, :dir, :draggable, :hidden, :lang, :spellCheck, :style, :tabIndex, :title, :loading_state]
        wild_props = Symbol[Symbol("data-"), Symbol("aria-")]
        return Component("html_data", "Data", "dash_html_components", available_props, wild_props; kwargs...)
end

html_data(children::Any; kwargs...) = html_data(;kwargs..., children = children)
