...
function html_mapel(; kwargs...)
        available_props = Symbol[:children, :id, :n_clicks, :n_clicks_timestamp, :key, :role, :name, :accessKey, :className, :contentEditable, :contextMenu, :dir, :draggable, :hidden, :lang, :spellCheck, :style, :tabIndex, :title, :loading_state]
        wild_props = Symbol[Symbol("data-"), Symbol("aria-")]
        return Component("html_mapel", "MapEl", "dash_html_components", available_props, wild_props; kwargs...)
end

html_mapel(children::Any; kwargs...) = html_mapel(;kwargs..., children = children)
