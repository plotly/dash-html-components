...
function html_area(; kwargs...)
        available_props = Symbol[:children, :id, :accessKey, :alt, :className, :contentEditable, :contextMenu, :coords, :dir, :download, :draggable, :hidden, :href, :hrefLang, :key, :lang, :loading_state, :media, :n_clicks, :n_clicks_timestamp, :referrerPolicy, :rel, :role, :shape, :spellCheck, :style, :tabIndex, :target, :title]
        wild_props = Symbol[Symbol("aria-"), Symbol("data-")]
        return Component("html_area", "Area", "dash_html_components", available_props, wild_props; kwargs...)
end

html_area(children::Any; kwargs...) = html_area(;kwargs..., children = children)
