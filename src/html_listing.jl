...
"""
    html_listing(;kwargs...)
    html_listing(children::Any;kwargs...)
    html_listing(children_maker::Function;kwargs...)
...
function html_listing(; kwargs...)
        ...
        result = Component("Listing", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_listing"))
            end
...
function html_listing(children::Any; kwargs...)
    result = html_listing(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_listing(children_maker::Function; kwargs...) = html_listing(children_maker(); kwargs...)
