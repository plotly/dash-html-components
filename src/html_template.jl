...
function html_template(; kwargs...)
        ...
        result = Component("Template", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_template"))
            end
...
function html_template(children::Any; kwargs...)
    result = html_template(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_template(children_maker::Function; kwargs...) = html_template(children_maker(); kwargs...)
