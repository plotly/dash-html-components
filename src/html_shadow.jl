...
function html_shadow(; kwargs...)
        ...
        result = Component("Shadow", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_shadow"))
            end
...
function html_shadow(children::Any; kwargs...)
    result = html_shadow(;kwargs...)
