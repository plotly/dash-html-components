...
"""
    html_isindex(;kwargs...)
    html_isindex(children::Any;kwargs...)
    html_isindex(children_maker::Function;kwargs...)
...
function html_isindex(; kwargs...)
        ...
        result = Component("Isindex", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_isindex"))
            end
...
function html_isindex(children::Any; kwargs...)
    result = html_isindex(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_isindex(children_maker::Function; kwargs...) = html_isindex(children_maker(); kwargs...)
