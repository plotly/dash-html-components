...
function html_aside(; kwargs...)
        ...
        result = Component("Aside", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_aside"))
            end
...
function html_aside(children::Any; kwargs...)
    result = html_aside(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_aside(children_maker::Function; kwargs...) = html_aside(children_maker(); kwargs...)
