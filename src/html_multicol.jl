...
export html_multicol

"""
    html_multicol(;kwargs...)
    html_multicol(children::Any;kwargs...)
    html_multicol(children_maker::Function;kwargs...)
...
function html_multicol(; kwargs...)
        ...
        result = Component("Multicol", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_multicol"))
            end
...
function html_multicol(children::Any; kwargs...)
    result = html_multicol(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_multicol(children_maker::Function; kwargs...) = html_multicol(children_maker(); kwargs...)
