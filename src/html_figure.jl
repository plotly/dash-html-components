...
function html_figure(; kwargs...)
        ...
        result = Component("Figure", "dash_html_components", Dict{Symbol, Any}(), available_props, Set(Symbol[Symbol("data-"), Symbol("aria-")]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "html_figure"))
            end
...
function html_figure(children::Any; kwargs...)
    result = html_figure(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

html_figure(children_maker::Function; kwargs...) = html_figure(children_maker(); kwargs...)
